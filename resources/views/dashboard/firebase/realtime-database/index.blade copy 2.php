...
@section('content')
    ...
    @push('js-internal')
        <script type="module">
            ...
            function fetchItemToTable(name, email, provider, user_image, uid) {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <a href="javascript:void(0)" class="d-flex align-items-center" style="cursor: default">
                            <img src="${user_image}" width="40" class="avatar rounded-circle me-3">
                            <div class="d-block ml-3">
                                <span id="namaUser" class="fw-bold name-user">${name}</span>
                                <div id="emailUser" class="small text-secondary">${email}</div>
                            </div>
                        </a>
                    </td>
                    <td>${provider}</td>
            ...
            function fetchAllItemToTable(users) {
                // tbody.innerHTML = '';
                users.forEach((user) => {
                    fetchItemToTable(user.name, user.email, user.provider, user.user_image, user.uid);
                });
            }

            function fetchAllDataRealtime() {
                ...
                onValue(dbRef, (snapshot) => {
                    ...
                    snapshot.forEach(childSnapshoot => {
                        // key
                        let key = childSnapshoot.key;
                        let value = childSnapshoot.val();
                        users.push(value);
                    });

                    fetchAllItemToTable(users);
                });
            }

            window.addEventListener('load', () => {
                fetchAllDataRealtime();
            });
            ...
            function insert() {
                ...
                // set(ref(db, 'users/' + $('#add_uid').val()), data).then(() => {
            ...
            function deleteData() {
                ...
                remove(ref(db, 'users/' + uid)).then(() => {
                    alertify.delay(4500).log(`Data user "${name}" berhasil dihapus`);
