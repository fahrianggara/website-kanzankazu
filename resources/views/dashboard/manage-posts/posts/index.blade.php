...
@section('content')
    ...
    <div class="row">
                                ...
                                @foreach ($post->tutorials as $tutorial)
                                    <div class="ribbon-wrapper ribbon-lg">
                                        @if ($tutorial->id == '1')
                                            ...
                                            @elseif ($tutorial->id == '2')
                                                <div class="ribbon bg-danger">
                                                @elseif($tutorial->id == '3')
                                                    <div class="ribbon bg-info">
                                                    @elseif ($tutorial->id == '4')
        ...
        <img src="{{ asset('vendor/blog/img/default.png') }}" alt="{{ $post->title }}" class="card-img-top img-fluid">
        @endif
        <div class="card-body">
            {{-- text --}}
            ...
            @can('post_detail')
                @if ($post->title != null && $post->description != null || $cateOld != null)
                    <a href="{{ route('posts.show', ['slug' => $post->slug]) }}"
                        class="btn btn-primary btn-sm waves-effect waves-light" data-toggle="tooltip" data-placement="bottom"
                        title="Lihat">
            ...
            @if ($post->user_id == Auth::user()->id)
                ...
                @can('post_update')
                    <a href="{{ route('posts.edit', ['slug' => $post->slug]) }}"
                        class="btn btn-warning btn-sm waves-effect waves-light" data-toggle="tooltip" data-placement="bottom"
                        title="Edit">
                ...
                @can('post_delete')
                    ...
                    <form action="{{ route('posts.destroy', ['post' => $post]) }}" method="POST" class="d-inline"
                        ...
                        <button type="submit" class="btn btn-danger btn-sm waves-effect waves-light" data-toggle="tooltip"
                ...
                @if ($post->status == 'publish')
                    {{-- DRAFT --}}
                    @if ($post->title != null && $post->description != null || $cateOld != null)
                        <form action="{{ route('posts.draft', ['post' => $post]) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
            ...
            @if ($post->status == 'approve')
                @if (!Auth::user()->editorRole())
                    <form action="{{ route('posts.approval', ['post' => $post]) }}" method="POST" class="d-inline"
