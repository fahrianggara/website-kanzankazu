...
@section('content')
    <div class="row">
        ...
        <div class="col-12">
            <div class="card m-b-30">
                <div id="fetchTableProject" class="card-body table-responsive shadow-sm table-wrapper"></div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-centered">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalCreate">Buat Title Project</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>

                <form id="formCreate" action="{{ route('project.store') }}" autocomplete="off" method="POST">
                    @csrf
                    @method('POST')
                    ...
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" id="add_title" name="title"
                                placeholder="Masukkan title project yang kamu buat">
                            <span class="invalid-feedback d-block error-text add_title_error"></span>
                        </div>
                        <div class="form-group mb-3">
                            <label for="slug">Slug Project</label>
    ...
    <div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="modalEdit" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-centered">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalEdit">Buat Title Project</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchProjects() {
                $.ajax({
                    ...
                    success: function(response) {
                        $("#fetchTableProject").html(response);

                        let dataTable = $('#tableProject').DataTable({
                            "ordering": false,
                            "bInfo": false,
                            "pageLength": 10,
                            "order": [
            ...
            $(document).on('click', '.edit_btn', function(e) {
                ...
                $.ajax({
                    url: "{{ url('dashboard/project/edit') }}/" + id,
                    method: "GET",
                    success: function(response) {
                        $('#edit_id').val(id);
            ...
            $('#formEdit').on('submit', function(e) {
                ...
                $.ajax({
                    method: "PUT",
                    url: "{{ url('dashboard/project/update') }}/" + id,
                    data: {
            ...
            $(document).on('click', '.del_btn', function(e) {
                ...
                $('#del_id').val(id);
                $('#text_del').text('Apakah kamu yakin ingin menghapus judul ' + title + '?');
            });
            ...
            $('#formDelete').on('submit', function (e) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "{{ url('dashboard/project/destroy') }}/" + id,
                    data: {
                        id: id,
                    },
                    beforeSend: function() {
